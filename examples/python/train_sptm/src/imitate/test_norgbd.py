...
def test_scenario(model, wad_path, vid_out_name=None):
    ...
    for step in range(num_steps - 1):
        ...
        out = model.predict_on_batch(batch_ga)
        pred_action = np.argmax(out, axis=1)[0]
        binary_action = [int(x) for x in list('{0:03b}'.format(pred_action))]
        print(out)
        print(pred_action)
        print(binary_action)
        action = [0.0] * 21
        action[7] = binary_action[0]
        action[8] = binary_action[1]
        action[9] = binary_action[2]
...
model_path = '../../experiments/trained_models/model_norgbd_softmax.100.h5'
