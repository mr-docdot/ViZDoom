...
def explore(config, scenario, episode, lmp_out_dir='.', vid_out_name=None):
    game = vzd.DoomGame()
    # Use your config
    ...
    lmp_path = os.path.join(lmp_out_dir, scenario_name + '_' + str(episode) +
                            '_rec.lmp')
    game.new_episode(lmp_path)
    ...
    while not game.is_episode_finished():
        ...
        vis_map, simple_map, curr_goal = compute_map(state,
                                                     pick_new_goal=pick_new_goal, # NOQA
                                                     curr_goal=curr_goal, # NOQA
                                                     explored_goals=explored_goals) # NOQA
...
if __name__ == "__main__":
    explore(DEFAULT_CONFIG, '../data/Test/random1/random1.wad', 99,
            vid_out_name='debug.avi')
